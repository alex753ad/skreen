...
def recommend_position_size(quality_score, confidence, entry_readiness,
    ...
    """v23.0: Recommend position size $25-$100 based on quality metrics."""
    # Base multiplier by quality + confidence
    ...
    if correlation < 0.3:
        mult *= 0.85
    size = max(25.0, round(base_size * mult / 5) * 5)  # round to $5
...
def add_position(coin1, coin2, direction, entry_z, entry_hr, 
    ...
    pos = {
        ...
        'entry_z': entry_z,
        'entry_hr': entry_hr,
        'entry_intercept': entry_intercept,  # v23.0: for static spread
...
def monitor_position(pos, exchange_name):
    ...
    pnl_pct = pnl_gross - COMMISSION_ROUND_TRIP_PCT  # v23.0: after commission
...
# Tabs
tab1, tab2, tab_phantom, tab3, tab4 = st.tabs([f"📍 Открытые ({len(open_positions)})", 
